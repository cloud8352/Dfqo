...
local function _NewPoint(id, x, y, dx, dy, pressure)
    if (_pointMap[id]) then
        ...
        _pointMap[id] = { x = x, y = y, dx = dx, dy = dy, pressure = pressure, moving = true, IsPressed = _IsPressed, IsReleased =
            _IsReleased, IsHold = _IsHold }
    end
end
...
function _TOUCH.WhetherExistHoldPoint()
    for id, point in pairs(_pointMap) do
        if (point ~= nil) then
            if (_IsHold(point)) then
                return true
            end
        end
    end
...
---@param point Lib.TOUCH.Point
function _TOUCH.WhetherPointIsHold(point)
