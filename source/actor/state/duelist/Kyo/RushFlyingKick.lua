--[[
	desc: RushFlyingKick, a state of Kyo.
	author: keke
]]
--
...
---@class Actor.State.Duelist.Kyo.RushFlyingKick : Actor.State
...
local RushFlyingKick = require("core.class")(_Base)

local HitStop = { 120, 110 }

function RushFlyingKick:Ctor(data, ...)
    _Base.Ctor(self, data, ...)
end

function RushFlyingKick:Init(entity)
...
function RushFlyingKick:NormalUpdate(dt, rate)
...
function RushFlyingKick:Enter(laterState, skill)
    if (laterState ~= self) then
        ...
        self.easeMoveX:Enter("x", 10, 0.15, direction)
        self.easeMoveZ:Enter("z", 11, 0.4, -1)
...
function RushFlyingKick:Exit(nextState)
