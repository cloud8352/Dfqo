--[[
	desc: GhostBurning, a state of Iori.
	author: keke
]]
--
...
---@class Actor.State.Duelist.Iori.GhostBurning : Actor.State
...
local GhostBurning = require("core.class")(_Base)

local HitStop = { 220, 110 }

function GhostBurning:Ctor(data, ...)
    _Base.Ctor(self, data, ...)
end

function GhostBurning:Init(entity)
...
function GhostBurning:NormalUpdate(dt, rate)
    ...
    if (tick == 4) then
        ...
        self.easeMoveX:Enter("x", 5, 0.35, direction)

        self.easeMoveZ:Enter("z", 5, 0.25, -1)
    ...
    if false == self.easeMoveZ.isRunning then
        
    end
    _STATE.AutoPlayEnd(self._entity.states, self._entity.aspect, self._nextState)
end

function GhostBurning:Enter(laterState, skill)
...
function GhostBurning:Exit(nextState)
