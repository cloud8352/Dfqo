--[[
	desc: SkillEnhanceShopWidget class. 技能增强商店控件
...
---@class SkillEnhanceShopWidget : Widget
local SkillEnhanceShopWidget = require("core.class")(Widget)
...
---@param model UiModel
function SkillEnhanceShopWidget.Create(parentWindow, model)
    -- 用于定义构造函数，解释使用，不做实际用途
    -- 使用class模块后，实际会调用Ctor函数
    return SkillEnhanceShopWidget.New(parentWindow, model)
end

---@param parentWindow Window
---@param model UiModel
function SkillEnhanceShopWidget:Ctor(parentWindow, model)
    -- 父类构造函数
    ...
    self.cdReduceBtn = PushButton.Create(parentWindow)
    self.cdReduceBtn:SetText("冷却时间-5%")

    self.priceLabel = Label.Create(parentWindow)
    self.priceLabel:SetText("需要100个太素")
    ...
    self.cdReduceBtn:MocConnectSignal(self.cdReduceBtn.Signal_BtnClicked, self)

    -- post init
end

function SkillEnhanceShopWidget:Update(dt)
    self:MouseEvent()

    if (Widget.IsSizeChanged(self)
        )
    then
        self:updateSelectedSkillContent()
    end
    ...
    self.cdReduceBtn:Update(dt)
    self.priceLabel:Update(dt)

    self.lastSelectedSkillItem = self.selectedSkillItem
    Widget.Update(self, dt)
end

function SkillEnhanceShopWidget:Draw()
    Widget.Draw(self)
    ...
    self.cdReduceBtn:Draw()
    self.priceLabel:Draw()
end

function SkillEnhanceShopWidget:MouseEvent()
end

---@param x int
---@param y int
function SkillEnhanceShopWidget:SetPosition(x, y)
    ...
    self.cdReduceBtn:SetPosition(x + LeftPartWidth, 
        y + selectedSkillTitleLabelHeight + selectedSkillContentScrollAreaHeight)


    local cdReduceBtnX, cdReduceBtnY = self.cdReduceBtn:GetPosition()
    local cdReduceBtnWidth, cdReduceBtnHeight = self.cdReduceBtn:GetSize()
    self.priceLabel:SetPosition(cdReduceBtnX + cdReduceBtnWidth, cdReduceBtnY)
end

---@param width int
---@param height int
function SkillEnhanceShopWidget:SetSize(width, height)
    ...
    self.cdReduceBtn:SetSize(150 * windowSizeScale, 50 * windowSizeScale)

    self.priceLabel:SetSize(150 * windowSizeScale, 50 * windowSizeScale)

    local selectedSkillTitleLabelHeight = 30 * Util.GetWindowSizeScale()
    local _, cdReduceBtnHeight = self.cdReduceBtn:GetSize()
    ...
    self.selectedSkillContentScrollArea:SetSize(width - LeftPartWidth,
        height - selectedSkillTitleLabelHeight - cdReduceBtnHeight)
end

function SkillEnhanceShopWidget:SetEnable(enable)
...
---@param sender Obj
function SkillEnhanceShopWidget:Slot_PlayerChanged(sender)
    ...
    for _, info in pairs(skillInfoList) do
        print("SkillEnhanceShopWidget:Slot_PlayerChanged()", info.iconPath, info.name)
...
function SkillEnhanceShopWidget:Slot_PlayerMasteredSkillAdded(sender, info)
    self:appendItem(info)
end

---@param sender Obj
---@param info SkillInfo
function SkillEnhanceShopWidget:Slot_PlayerMasteredSkillChanged(sender, info)
...
function SkillEnhanceShopWidget:Slot_SelectedItemChanged(sender, item)
    print("SkillEnhanceShopWidget:Slot_SelectedItemChanged(item)", item:GetIndex())

    self.selectedSkillItem = item
end

---@param sender Obj
function SkillEnhanceShopWidget:Slot_BtnClicked(sender)
    print("SkillEnhanceShopWidget:Slot_BtnClicked()")
    if sender == self.cdReduceBtn then
        ---@type SkillInfo
        local info = self.selectedSkillItem:GetValue(ItemDataKey)
        self.model:PayForReduceCdTime(info.resDataPath)
    end
end
...
--- 更新被选择技能项内容
function SkillEnhanceShopWidget:updateSelectedSkillContent()
    ...
    --- 判断冷却时间减少按钮的可用状态
    if info.cdTime < 1000 then
        self.cdReduceBtn:SetEnable(false)
    else
        self.cdReduceBtn:SetEnable(true)
    end
end

---@param info SkillInfo
function SkillEnhanceShopWidget:appendItem(info)
...
---@param info SkillInfo
function SkillEnhanceShopWidget:updateItemByInfo(item, info)
