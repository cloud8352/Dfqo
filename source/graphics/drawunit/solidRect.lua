...
function _SolidRect:GetStruct(name)
    if (name) then
        ...
        return self._struct.x, self._struct.y1, self._struct.y2,
            self._struct.z, self._struct.w, self._struct.h
    end
end
...
function _SolidRect:Compare(another, onlyStruct)
    ...
    local structIsComparable =
        self._struct.x == x and
        self._struct.y1 == y1 and self._struct.y2 == y2 and
        self._struct.z == z and self._struct.w == w and self._struct.h == h
    ...
    if (onlyStruct) then
        ...
        return structIsComparable and
            x == self._x and y == self._y and
            z == self._z and sx == self._sx and
            sy == self._sy and r == self._r
    end
end
