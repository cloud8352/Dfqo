...
local PathFinderApi = require("util.PathFinderApi")
...
---@field protected pathFinder Util.PathFinderApi
...
function _Matrix:Ctor(gridSize, x, y, w, h)
    ...
    self.pathFinder = PathFinderApi.New("JPS")
...
function _Matrix:Reset(x, y, w, h, initOpenNodes)
    ...
    self.pathFinder:Reset(self:ToNode(w), self:ToNode(h))
...
function _Matrix:MakeSprite()
    ...
    for n = 1, self.pathFinder:GetWidth() do
        for m = 1, self.pathFinder:GetHeight() do
            if (self.pathFinder:GetNode(n, m)) then
...
function _Matrix:CreatePlaneMapSprite()
    ...
    for n = 1, self.pathFinder:GetWidth() do
        for m = 1, self.pathFinder:GetHeight() do
            if (false == self.pathFinder:GetNode(n, m)) then
                _GRAPHICS.DrawRect(n * self._gridSize, m * self._gridSize, self._gridSize, self._gridSize, "fill")
            end
        end
    end
...
function _Matrix:SetNode(x, y, isObs, isOrigin)
    ...
    self.pathFinder:SetNode(x, y, isObs)
end
...
function _Matrix:SetNodeWithRect(rect, isObs)
    ...
    for n = x, xw do
        for m = y, yh do
            self.pathFinder:SetNode(n, m, isObs)
        end
    end
end
...
function _Matrix:GetNode(x, y, isOrigin)
    ...
    return self.pathFinder:GetNode(x, y)
end
...
function _Matrix:GetOpenNode(x, y, isOrigin, toPosition)
    ...
    local n, m = self.pathFinder:GetOpenNode(x, y)
...
function _Matrix:GetPath(x1, y1, x2, y2)
    ...
    x2 = self:ToNode(x2, "x")
    y2 = self:ToNode(y2, "y")
    local path = self.pathFinder:GetPath(x1, y1, x2, y2)
...
function _Matrix:GetSimplePath(x1, y1, x2, y2)
    ...
    local path = {}
    local point = _Point.New(true, x2, y2)
...
function _Matrix:ToNode(value, type, correct)
    ...
    local v = math.floor(value / self._gridSize) + 1
...
function _Matrix:GetWidth(isOrigin)
    local width = self.pathFinder:GetWidth()
...
function _Matrix:GetHeight(isOrigin)
    local height = self.pathFinder:GetHeight()
